...
void ButtonCallback(const kobuki_msgs::ButtonEvent::ConstPtr& msg){
    ...
    if (Pressed == true){
        if (Button == 0){
            /*The coordinates stored in "xB228" and "yB228" is passed 
            over to Goal coordinates. */
            xGoal = xB228;
            yGoal = yB228;
            zGoal = zB228;
            wGoal = wB228;
            ...
            /*The coordinates stored in "xPrinterRoom" and "yPrinterRoom" 
            is passed over to Goal coordinates. */
            xGoal = xPrinterRoom;
            yGoal = yPrinterRoom;
            zGoal = zPrinterRoom;
            wGoal = wPrinterRoom;
        ...
        /*Setting a timer, so the 'action server' can come up , and it 
        will wait for mak. 5 sec. */
...
int main(int argc, char** argv){
    ...
    Evertime an advertisment is made on the topic "mobile_base/events/button" 
    run "ButtonCallBack"*/
    ros::Subscriber Button_sub = n.subscribe("mobile_base/events/button", 
    1, ButtonCallback);
...
void light(int led, int color){
    ...
    switch (color){
        case 's':
            /*If the variable "color" has the value "s", the color is set to 
            NULL (which translate to the led turned off) */
            asignedColor = 0;
            break;

        case 'g':
            /*If the variable "color" has the value "g", the color is set to 
            GREEN. */
            asignedColor = 1;
            break;

        case 'o':
            /*If the variable "color" has the value "o", the color is set to 
            ORANGE. */
            asignedColor = 2;
            break;

        case 'r':
            /*If the variable "color" has the value "r", the color is set to 
            RED. */
            asignedColor = 3;
            break;
    ...
    switch(led){
        case 1:
            /*If the variable "led" has the value 1, the Led 1 will light up, 
            with the asigned color from the switch above. */
            led1_pub.publish(led_message);
            break;
        case 2:
            /*If the variable "led" has the value 2, the Led 2 will light up, 
            with the asigned color from the switch above. */
